...
function Calendars(props) {
  ...
  const contentHandler = ({ activeStartDate, date, view }) => {
    ...
    const isDate = props.reservations.some(
      (a) => a.reservation_date === currentDate
    ); //map은 true를 반환해도 계속 돌기에 some을 사용하여 true에서 멈추는 로직 사용
  ...
  return (
    <>
      ...
      >
        ...
        <h4>
          {
            props.reservations.find((a) => a.reservation_date === clickedDate)
              ?.reservation_date
          }
        </h4>
        <h4>
