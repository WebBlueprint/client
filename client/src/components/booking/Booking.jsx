...
const Booking = () => {
  ...
  const handleLikeClick = () => {
    if (selectedProIndex !== null) {
      const updatedLikedPros = likedPros.some((pro) => pro.index === selectedProIndex)
        ? likedPros.filter((pro) => pro.index !== selectedProIndex)
        : [...likedPros, { index: selectedProIndex, name: proboxData[selectedProIndex].Proname, liked: true }];
  ...
  useEffect(() => {
    const storedLikedPros = JSON.parse(sessionStorage.getItem("likedPros")) || [];
      ...
      <div className={styles.cover}>
        ...
        <div className={styles.bookingContainer}>
          <div className={styles.iconwrap}>
            <div onClick={handlePrev} className={styles.iconwrap1}> <Playback /> </div>
            <div onClick={handleNext} className={styles.iconwrap2}> <Play /> </div>
          </div>
          <div
          ...
          >
            {proboxData.map((item, index) => (
              <div
                key={index}
                className={`${styles.bookingItem} ${selectedProIndex === index && styles.selectedPro}`}
      ...
      <span className={styles.likeButtontext} >Add My Pro</span>
        ...
        {selectedProIndex !== null && (
          <div className={styles.schedulebox}>
            ...
            <button className={styles.btnbooking} onClick={handleBooking}>Booking</button>
          </div>
        )}
      </div>
      {modalVisible && (
        <div className={styles.modal}>
          <div className={styles.modalContent}>
            ...
            {selectedProIndex !== null && selectedDateIndex !== null && selectedTimeIndex !== null && (
              <div className={styles.bookingInfo}>
                <p>You have successfully booked an appointment with {proboxData[selectedProIndex].Proname}.</p>
                <p>Date: {nextDates[selectedDateIndex]} <b>{proboxData[selectedProIndex].availableSlot.reservedTimes[selectedTimeIndex]}</b> </p>
              </div>
            )} 
          </div>
        </div>
